...
class Tasks:
    ...
    def task1(self):
        try:
            ...
            try:
                ...
                write_proc = self.start_server(
                    "./write_server", self.write_port)
            except:
                print(
                    '\033[91m'+'task1: start read_server/write_server failed.'+'\033[0m')
                raise
            else:
                try:
                    # print()
    ...
    def task6(self):
        ...
        read_proc = self.start_server("./read_server", 10000)
        read_proc2 = self.start_server("./read_server", 10001)
        write_proc = self.start_server("./write_server", self.write_port)
        ...
        try:
            ...
            telnet_client3 = TelnetConnection("localhost", 10000)
            telnet_client3 = telnet_client3.telnet()
            telnet_client4 = TelnetConnection("localhost", 10001)
            telnet_client4 = telnet_client4.telnet()
        ...
        try:
            gets = telnet_client.read_until(b"\n", timeout=0.2).decode()
            print("\33[3m"+"(Write Server output)"+gets)

            gets = telnet_client2.read_until(b"\n", timeout=0.2).decode()
            print("\33[3m"+"(Write Server2 output)"+gets)

            gets = telnet_client3.read_until(b"\n", timeout=0.2).decode()
            print("\33[3m"+"(Read Server3 output)"+gets)

            gets = telnet_client4.read_until(b"\n", timeout=0.2).decode()
            print("\33[3m"+"(Read Server4 output)"+gets)

            print("\33[3m"+"(Client3 side input) " + "\33[0m"+"902001")
            telnet_client3.write(b"902001\n")

            print("\33[3m"+"(Client4 side input) " + "\33[0m"+"902001")
            telnet_client4.write(b"902001\n")
            ...
            gets = telnet_client3.read_until(b"\n", timeout=0.2).decode()
            print("\33[3m"+"(Write Read Server3 output)"+gets)

            gets = telnet_client4.read_until(b"\n", timeout=0.2).decode()
            print("\33[3m"+"(Write Read Server4 output)"+gets)
    ...
    try:
        ...
        try:
            ...
            cmd = subprocess.Popen(
                ["cp", "registerRecord_copy", "registerRecord"])
        try:
            print(
